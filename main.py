...
class App(CTk):
    ...
    def begin_installation(self):
        ...
        if not DEBUG: self._execute(f"cryptsetup luksFormat {self.setup_information["SystemPartition"]}")
        if not DEBUG: self._execute(f"cryptsetup luksOpen {self.setup_information["SystemPartition"]} cryptlvm")
